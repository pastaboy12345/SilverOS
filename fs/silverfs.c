 ...
 * - Block 1: Inode Bitmap (offset 4096, size 4096 / 8 sectors. 1024 bits = 128 bytes used)
 * - Block 2: Block Bitmap (offset 8192, size 4096 / 8 sectors. 4096 bits = 512 bytes used)
...
static void read_block(uint32_t block, void *buf) {
    ata_read_sectors(BLOCK_TO_LBA(block) + silverfs_lba_offset, SECTORS_PER_BLOCK, (uint8_t *)buf);
}

static void write_block(uint32_t block, const void *buf) {
    ata_write_sectors(BLOCK_TO_LBA(block) + silverfs_lba_offset, SECTORS_PER_BLOCK, (const uint8_t *)buf);
}
...
static void read_inode(int idx, silverfs_inode_t *inode) {
    if (idx < 0 || idx >= SILVERFS_MAX_INODES) return;
...
static void bmp_clear(uint8_t *bmp, int idx) { bmp[idx/8] &= ~(1 << (idx%8)); }
static bool bmp_test(uint8_t *bmp, int idx)  { return bmp[idx/8] & (1 << (idx%8)); }
...
static int alloc_block(void) {
    for (int i = 0; i < SILVERFS_MAX_BLOCKS; i++) {
        ...
        if (i < (int)superblock.data_block_start) continue;
...
static int find_in_dir(int dir_inode_idx, const char *name) {
    ...
    if (dir.type != SILVERFS_TYPE_DIR) return -1;
...
static int resolve_path(const char *path) {
    ...
    if (path[0] == '/' && path[1] == 0) return superblock.root_inode;
...
int silverfs_format(void) {
    /* Since mkfs_svd formats the disk on the host, we should rarely need this in kernel.
     * It's a stub just in case we need to reformat a corrupted disk.
     */
    serial_printf("[SilverFS] Format requested, but we rely on mkfs_svd host tool. Skipping.\n");
    return -1;
}
...
int silverfs_mount(void) {
    ...
    if (superblock.magic == SILVERFS_MAGIC) goto mount_ok;
    ...
    silverfs_lba_offset = 0;
    serial_printf("[SilverFS] ERROR: Invalid magic. Disk not formatted properly.\n");
    return -1;
...
int silverfs_create(const char *path, uint32_t type) {
    ...
    if (find_in_dir(parent, basename) >= 0) return -1;
...
int silverfs_write(silverfs_file_t *file, const void *buf, size_t count) {
    ...
    while (bytes_written < count) {
        ...
        if (block_idx >= SILVERFS_DIRECT_BLOCKS) break;
        ...
        uint8_t bbuf[SILVERFS_BLOCK_SIZE];
        read_block(inode.blocks[block_idx], bbuf); /* Read to preserve other data in block */
